...
FILE_BUILD_GRADLE="$SELF_ROOT/cliniasearch/common.gradle"
VERSION_CODE=$1
...
if [ $COUNT_DOTS -ne 2 ]; then
    echo '$VERSION_CODE is not aa valid version coe, please use the form X.Y.Z (e.g. v1 = 1.0.0)'
    exit -1
fi
...
do
    ...
    if [[ $flavor = "online" ]]; then
        module_name="cliniasearch-android"
    elif [[ $flavor = "offline" ]]; then
        module_name="cliniasearch-offline-android"
    fi
...
git checkout $SELF_ROOT/cliniasearch/build.gradle
